...
export async function POST(req: Request) {
  try {
    ...
    // Parse the request body to get store_id
    ...
    // Query to get all orders where confirm_payments is true and nearest_store_id matches
    const [rows] = await pool.query<mysql.RowDataPacket[]>(`
      SELECT 
        ...
        delivered_date_time 
      FROM orders 
      WHERE confirm_payments = true AND nearest_store_id = ?
    `, [store_id]);
