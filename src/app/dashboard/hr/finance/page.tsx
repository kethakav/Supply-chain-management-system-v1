...
interface ManagerData {
    finance_manager_id: number; // Unique identifier for the manager
...
export default function HRDashboard() {
    ...
    useEffect(() => {
        const fetchManagers = async () => {
            try {
                const response = await fetch('/api/hr/get-finance-managers');
                if (!response.ok) {
    ...
    const handleRemoveManager = async (id: number) => {
        ...
        try {
            const response = await fetch('/api/hr/remove-managers', {
                ...
                body: JSON.stringify({
                    input_table: 'financeandorder_manager',
                    input_id: id,
                }),
            });
            ...
            setManagers((prevManagers) => prevManagers.filter(manager => manager.finance_manager_id !== id));
    ...
    const handleAddManager = () => {
        router.push("/dashboard/hr/finance/add-account"); // Redirect to the add account page
    };
    ...
    return (
        <DashboardLayout>
            <Breadcrumb pageName="Finance and Order Management" />
            <div className="rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:bg-gray-dark dark:shadow-card">
                ...
                <div className="flex flex-col">
                    <div className="grid grid-cols-5 sm:grid-cols-5"> {/* Change columns to 5 for additional data */}
                    ...
                    {managers.map((manager, key) => (
                        <div
                            ...
                            key={manager.finance_manager_id}
                        >
                            ...
                            <div className="flex items-center justify-center px-2 py-4">
                                <ButtonDefault 
                                    label="Remove" 
                                    onClick={() => handleRemoveManager(manager.finance_manager_id)} // Hooking the button to the remove functionality
