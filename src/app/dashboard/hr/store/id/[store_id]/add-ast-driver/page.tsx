...
const AddStoreManager = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/auth/ast-driver/signup", {
        method: "POST",
        headers: {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName={`Add Assistant Driver for Store ${params.store_id}`} />
            ...
            <div className="rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
              <div className="border-b border-stroke px-6.5 py-4 dark:border-dark-3">
                <h3 className="font-medium text-dark dark:text-white">
                  Assistant Driver Information for Store {params.store_id}
                </h3>
              </div>
              <div className="flex flex-col gap-5.5 p-6.5">
                ...
                <button
                  ...
                  disabled={isLoading}
                >
                  {isLoading ? "Adding Assistant Driver..." : "Add Assistant Driver"}
                </button>
              </div>
            </div>
          </div>
